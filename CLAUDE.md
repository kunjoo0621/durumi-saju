...
- **v20→v21 변경 (2026-08-27)**: 극왕에 관살이 없으면 관성이 **구조적으로** 용신이 되던 문제.
  ...
  - ★"관살 0"만으로 종왕을 선언하지 않는다. **"四柱皆比劫"은 문자 그대로** — 같은 장의 종왕 실례 **癸卯 乙卯 甲寅 乙亥**(목6·수2)가 식상·재성·관성 전부 0이다. 재성은 임철초가 **"遇財星, 群劫相爭, 九死一生"**이라 한 배치이고, 식상도 없어야 한다
  - ※임철초 "局中印輕, 行傷食亦佳"는 **운(運)에서** 식상이 올 때 좋다는 말이지 원국 조건이 아니다. 초안이 이를 원국 조건으로 오독해 식상 검사를 뺐다가 리뷰에서 잡혔다(당시 28명 중 22명이 식상 보유)
  - 갈래 둘: **종왕**(극왕+관살0+재성0+**식상0**+인수≥1, **7명**) → 용신 비겁·희신 인성·기신 관성 / **관성 제외**(나머지 **46명**) → {식상·재성} 중 최저
  - 실측(region 경도보정 반영, 3,288명): 용신 변경 **53명**, **그 밖에서 바뀐 인원 0명**(변경 국소성 전수 증명), composite 상승 13·**하락 0**·동일 40(평균 +0.42), 등급 이동 **0명**
  - ★조후 노트와의 교집합: 종왕 기신은 관성인데 게이트가 관살 0을 요구해 조후 오행 개수가 **항상 0** → "조후위급, 채워라" 분기에 떨어진다. 犯旺 오행을 채우라는 지시가 되므로 **종왕 전용 문구로 분리**했다(우선순위는 조후위급보다 종격 판정이 앞선다 — 종격은 억부·조후를 적용할 국면 자체를 바꾸는 판정이다). 식상 조건 추가 후 실사용자 교집합은 0명이나 이론상 도달 가능하므로 방어 코드로 유지
