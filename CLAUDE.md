...
- **v20→v21 변경 (2026-08-27)**: 극왕에 관살이 없으면 관성이 **구조적으로** 용신이 되던 문제.
  ...
  - 갈래 둘: **종왕**(극왕+관살0+재성0+인수≥1, 28명) → 용신 비겁·희신 인성·기신 관성 / **관성 제외**(나머지 25명) → {식상·재성} 중 최저
  - 실측(region 경도보정 반영, 3,287명): 용신 변경 **53명**, **그 밖에서 바뀐 인원 0명**(변경 국소성 전수 증명), composite 상승 34·**하락 0**·동일 19(평균 +1.04), 등급 상승 1·**하락 0**
  - ★조후 노트와의 교집합: 종왕 기신은 관성인데 게이트가 관살 0을 요구해 조후 오행 개수가 **항상 0** → 전원이 "조후위급, 채워라" 분기에 떨어졌다(6명). 犯旺 오행을 채우라는 지시가 되므로 **종왕 전용 문구로 분리**. 우선순위는 조후위급보다 종격 판정이 앞선다 — 종격은 억부·조후를 적용할 국면 자체를 바꾸는 판정이다
