...
// 결혼운 자체입력(self) 미리보기 화면 — app/teaser/page.tsx 미러(원국 무료 공개 + 등급 잠금 + 결제 CTA).
// 대표사주 경로(/marriage/input)와 달리 여기서는 방금 입력한 생년월일(useInputStore)로
// 원국을 클라이언트에서 계산해 무료로 보여주고, source:"self"로 결혼운 start/analyze를 호출한다.
//
...
import { useMarriageStore, hasMarriageHydrated } from "@/store/useMarriageStore";
...
export default function MarriageTeaserPage() {
  ...
  // store hydration 대기 (useInputStore + useMarriageStore) — persist 값 로드 전 판단 금지.
  const [hydrated, setHydrated] = useState(false);
  useEffect(() => {
    const check = () => {
      if (hasInputHydrated() && hasMarriageHydrated()) {
        setHydrated(true);
        return true;
      }
      return false;
    ...
    const unsub2 = useMarriageStore.persist.onFinishHydration(() => check());
    return () => { unsub1(); unsub2(); };
  }, []);

  // 입력 검증 — 서버 필수(birth·gender) + 관계상태. 부족하면 자체입력으로 되돌린다.
  ...
  // 서버가 buildInputHash에 쓰는 필드 전부 + maritalStatus를 담으므로, 정상 재진입(입력 수정·
  // 상태 변경)은 키가 바뀌어 정상 재요청되고, 참조만 새로 생기는 경우는 키가 같아 재요청이 불가능하다.
  ...
  const teaserKey = useMemo(
    () => JSON.stringify({ selfInput, maritalStatus }),
    [selfInput, maritalStatus],
  );
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated") {
      signIn("kakao", { callbackUrl: "/marriage/teaser" });
      return;
    }
    if (!hasRequiredInput) {
  ...
  useEffect(() => {
    if (!hydrated || !hasRequiredInput || !isAuthenticated || !maritalStatus) return;
    ...
    (async () => {
      ...
      try {
        const res = await fetch("/api/marriage/start", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ source: "self", selfInput, maritalStatus }),
        });
        const data = await res.json().catch(() => ({}));
        if (cancelled) return;
        if (!res.ok) {
          setError(data?.error || "결혼운 미리보기를 준비하지 못했어.");
  ...
  }, [hydrated, hasRequiredInput, isAuthenticated, maritalStatus, teaserKey, teaserRetry]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const runUnlock = useCallback(async (overrideBalance?: number) => {
    ...
    try {
      ...
      if (!startRes.ok) throw new Error(startData?.error || "결혼운 준비에 실패했어.");
  ...
  }, [maritalStatus, balance, selfInput, router, setBalance]);
  ...
  // charge-success 복귀 처리 — 충전 완료 후 자동 재시도 (today/marriage entry 패턴 미러).
  ...
  useEffect(() => {
    ...
    if (!hasRequiredInput || !maritalStatus) {
      afterChargeRanRef.current = true;
      window.history.replaceState({}, "", "/marriage/teaser");
      return;
    }
    afterChargeRanRef.current = true;
    setConfirming(true);
  ...
  }, [hydrated, isAuthenticated, hasRequiredInput, maritalStatus, runUnlock, setBalance]);
  ...
  const starChip = teaserFacts?.spouseStarAbsent
    ...
    : teaserFacts?.gwansalHonjap
      ? `${teaserFacts?.spouseStarType ?? "배우자성"} 혼재`
      : teaserFacts?.spouseStarType
        ? `${teaserFacts.spouseStarType} 뚜렷`
        : null;
  ...
  return (
    <div className="min-h-screen bg-background-primary text-text-primary animate-fadeIn">
      <Header showBack sticky onBack={() => router.push("/marriage/self")} />
      ...
      <main className="px-6 py-8 pb-40">
        <div className="max-w-[640px] mx-auto space-y-6">
          ...
          <div className="px-1 space-y-6 pt-2">
            ...
            >
              {paying ? "준비 중…" : `${MARRIAGE_COST}알로 결혼운 전체 보기`}
            </button>
          </div>
        </div>
      </main>
