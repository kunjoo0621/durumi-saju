...
export async function POST(request: NextRequest) {
  try {
    ...
    const rlMinute = checkRateLimit(`marriage_start:${userId}:m`, 20, 60_000);
    const rlHour = checkRateLimit(`marriage_start:${userId}:h`, 120, 60 * 60_000);
    if (!rlMinute.allowed || !rlHour.allowed) {
      console.warn("[RATE_LIMIT] /api/marriage/start", { userId });
