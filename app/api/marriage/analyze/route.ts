...
type AnalyzeBody = { maritalStatus?: string; source?: "primary" | "self"; selfInput?: SelfSajuInput };
...
export async function POST(request: NextRequest) {
  try {
    ...
    // primary는 아래 등급게이트·원국 재조립에서도 참조되므로 null 가드와 함께 상위 스코프에 둔다.
    ...
    try {
      ...
      if (source === "self" && selfComputed) {
        ...
        gender = normGender(primary!.gender);
