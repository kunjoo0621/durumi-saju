...
// (WealthResultClient.WEALTH_TO_INTERNAL_GRADE와 동일 — 컴포넌트 내부 상수라 여기 재현.)
...
export default function WealthTeaserPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setGrade((data?.grade ?? data?.teaser?.grade ?? null) as WealthGrade | null);
  ...
  const internalGrade: OverallGradeLabel = grade ? WEALTH_TO_INTERNAL_GRADE[grade] : "D";
              ...
              {teaserState === "loading" ? (
                ...
                <>
                  ...
                  <p className="mt-4 max-w-[380px] text-[15px] leading-[1.7] text-text-secondary break-keep">
                    재성 진단, 재를 담는 그릇, 돈이 들어오는 시기까지 — 지금은 등급만 공개돼 있어.
                  </p>
