...
export default function CareerTeaserPage() {
  ...
  // 서버가 buildInputHash에 쓰는 필드 전부 + situation을 담으므로, 정상 재진입(입력 수정·상황
  // 변경)은 키가 바뀌어 정상 재요청되고, 참조만 새로 생기는 경우는 키가 같아 재요청이 불가능하다.
  // 2026-07-29 사고: selfInput 참조가 렌더마다 새로 생겨 effect가 무한 재실행되며
  // /api/career/start에 5분간 22,674건이 나갔다. 1층(useShallow)이 원인을 없앴지만,
  // 이 키+ref 가드는 selector가 다시 깨져도 루프가 열리지 않게 하는 구조적 차단이다.
  const teaserKey = useMemo(
    () => JSON.stringify({ selfInput, situation }),
    [selfInput, situation],
  );
  ...
  }, [hydrated, hasRequiredInput, isAuthenticated, situation, teaserKey, teaserRetry]); // eslint-disable-line react-hooks/exhaustive-deps
