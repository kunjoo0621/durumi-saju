...
export const MOCK_PET_DOG_A: PetResultData = {
  ...
  full_result: {
    ...
    simulations: [
      { scene: "산책 가자니까 우주 정복 나가는 표정", prediction: "현관에서 목줄만 들어도 두부는 꼬리부터 만렙으로 돈다. 그런데 조금만 덥거나 비가 오면 '왜 지금?' 하는 표정으로 너를 빤히 본다." },
      { scene: "초인종 한 번에 경비대장 빙의", prediction: "초인종이 울리면 몸집보다 큰 소리로 경비를 선다. 그러다 간식 봉지 소리 한 번에 태세를 바꿔 바로 영접 모드로 들어간다." },
      { scene: "혼나면 억울 대신 배부터 뒤집기", prediction: "혼나는 순간 눈이 잠깐 풀렸다가, '내가 뭘?' 하는 얼굴로 괜히 배를 뒤집어 상황을 무마하려 든다." },
    ],
