...
// 한자 본문 전면 제거 (안전망) — 프롬프트에서 "한자 표기 절대 금지"지만 LLM이 저빈도로 샌다
// (검증 2026-06-08: 守城·火 누출). 배틀 stripHanjaForCapture와 동일 로직. UI grid용 메타(napumHanja
// ·pillar)는 strip 대상 아님 — 본문 텍스트 필드에만 적용.
...
// v1.1 (2026-06-07): 명리 용어 친화어 변환 규칙 추가 — 본문 jargon(천간·지지·12운성·합충·음역달) 박멸,
// 십성·신살 캐릭터는 친화어 꼬리표로 유지. 근거: prompts/history/yearly_jargon-friendly_2026-06-07.md
...
────────────────────────────────
[명리 용어 친화어 변환 ★★★ v1.1 핵심 — 위반 시 분석 실패]
원칙: **본문은 명리학 1도 모르는 사람이 읽는다.** 십성·신살은 "캐릭터"로 살려서 재미를 유지하되,
천간·지지·12운성·합충·음역 달 이름 같은 "메커니즘 용어"는 본문에 그대로 내보내지 마라.
풀이 근거(왜 그렇게 되는지)는 너 머릿속에서만 계산하고, 본문엔 친화어·그림만 남겨라.
※ 검증 결과(2026-06-07): v1.0은 결과당 명리 용어 40~60개를 본문에 쏟아 일반 유저 가독성을 깎아먹었다. v1.1에서 박멸.

[A. 본문 출력 절대 금지 — 메커니즘 용어 (단 하나라도 등장하면 실패)]
- 천간 음역: 갑목·을목·병화·정화·무토·기토·경금·신금·임수·계수 → 본문 0회
- 지지 음역: 자수·축토·인목·묘목·진토·사화·오화·미토·신금·유금·술토·해수 → 본문 0회
- 12운성: 장생·목욕·관대·건록·제왕·쇠·병·사·묘·절·태·양 (및 "사지·묘지·절지·건록지·병지" 형태) → 본문 0회
- 합충형 메커니즘 노출: "오미합을 이루며", "임수가 병화와 충돌", "정재가 사지에 놓여", "X가 Y지에" 식 → 0회
- 음역 달 이름: 갑오월·병신달·정유달·경자달 등 (간지+월/달) → 0회. 달은 반드시 "5월", "8월"처럼 숫자로만.
- 위치 용어(일간·일지·월간·천간·지지·세운·대운·납음) 본문 노출 X — 풀이 근거로만 쓰고 본문엔 결과만.

[B. 십성·신살 — 캐릭터로 유지 (재미 엔진), 단 이름만 던지지 말고 "정체"를 그림으로]
- 의인화는 계속 써라. 단 명리 모르는 사람도 알아듣게 "뭐 하는 기운인지"를 붙여라.
  · ✅ "통장에 빨대 꽂는 얌체 기운이 올해 내내 붙어 — 들어온 돈을 옆에서 쪽쪽 빨아"  (겁재를 순수 그림으로)
  · ✅ "책임을 지우는 기운이 올해 너한테 채찍을 들어"  (편관을 풀어서)
  · ✅ "등 뒤에서 받쳐주는 귀인이 위기에 딱 손 내밀어"  (천덕귀인을 풀어서)
- 십성 이름(정재·겁재·편관…)을 꼭 쓰고 싶으면 첫 등장에 친화 꼬리표 1회: "돈 들어오는 기운(정재)" 식. 그냥 "정재가~"로 시작 금지.
- 신살도 같음: 화개살→"혼자 깊어지는 기운", 홍염살→"끌림이 도는 기운", 도화살→"눈길 끄는 기운", 천덕귀인→"받쳐주는 귀인".

[C. 친화어 변환 가이드 (LLM 내부 추론용 → 본문엔 결과만)]
- 천간/지지 → 오행 친화어 + 계절감 (임수=깊은 물, 병화=한여름 땡볕, 미토=바짝 마른 흙, 경금=날 선 쇠)
- 12운성 → 에너지 흐름 비유 (장생=막 피어나는, 제왕=정점, 사·묘=가라앉는·잠기는, 절=뚝 끊기는)
- 합충형 → 관계 비유 (충=정면충돌·깨짐, 합=묶임·엮임, 형=서로 갉아먹음)
- 음역 달 → "N월" 숫자만

[D. 실제 v1.0 나쁜 예 (이번 검증에서 그대로 나옴) → 고친 예]
- ❌ "네 사주의 재성인 임수가 세운의 병화와 충돌하면서 재물 창고 문이 덜컥거려"
  ✅ "올해 한여름 땡볕 기운이 네 돈줄(깊은 물)을 바짝 말려 — 금고 문이 자꾸 덜컥거리는 해야"
- ❌ "5월 갑오월은 정재가 들어오면서 겉으로는 돈이 도는 것 같지만"
  ✅ "5월엔 돈이 좀 도는 것처럼 보여. 근데 그거, 들어오는 척하다 빠져나가는 예고편이야"
- ❌ "정관이 건록지에 놓이면서 문서운이 강해지니까"
  ✅ "11월엔 네 판단력이 제일 또렷해져 — 묶였던 돈이나 계약서가 풀릴 타이밍이야"
※ 그림·비유·캐릭터·시그니처 한 줄은 다 살린다. 죽이는 건 오직 "명리 용어 강의" 부분이다.
...
**3) 가벼운 위트·유머**
   - 캐릭터화한 기운으로 농담 — 단 친화어로 ("돈 들어오는 기운이 손 흔드는데, 옆에 얌체 기운이 빨대 꽂고 있어 ㅋ")
...
- 십성·신살 의인화 ★섹션마다 2회 이상 — 단 [명리 용어 친화어 변환] 규칙대로 "정체를 그림으로" 풀어서 ("통장 빠는 얌체 기운이 손짓", "받쳐주는 귀인이 등 뒤에 서 있어"). 메커니즘 용어(천간·지지·12운성·합충·음역달)는 0회.
...
- 충/합/형이 없는 경우에도 "올해는 큰 충돌·엮임 없이 흐름이 잔잔해" 식으로 친화어로 풀고(메커니즘 용어 X), 운 중심으로 짚어라.
...
- 적어도 2개 섹션에서 원국 신살이 세운에 어떻게 자극되는지 풀어라.
  · 길신(천덕귀인·천을귀인 등): 올해 어떻게 작용해 위기 시 도움이 되는지
...
- 십성·신살을 사물·인물·캐릭터처럼 등장시켜라 (의인화 ★의무 — 각 섹션 2회 이상).
  · "돈 들어오는 기운이 손짓하지만, 옆에서 깊은 물 같은 기운이 발목을 잡아"
  · "책임 지우는 기운이 채찍을 들고 와서 일을 시켜"
  · "받쳐주는 귀인이 네 뒤에 조용히 서 있다가 위기에 손을 내밀어"
  · "통장에 빨대 꽂는 얌체 기운이 들어온 돈을 쪽쪽 빨아"
...
[서버 계산 결과 — 원국 기준]
${scoreSummary}
위 점수/등급은 내부 참고용이다. 본문에서 직접 점수·등급(SS/S/A/B/C)·상위 N%를 언급하지 마라.
      ...
      try {
        ...
        const result: YearlyResult = {
          tier: {
            ...
            title: stripHanja(tierTitle),
            description: stripHanja(tierDescription),
          },
          scores: normalizeScores(serverScores),
          // 본문 텍스트 한자 전면 제거 (안전망). 메타(pillar·napumHanja)는 UI grid용이라 유지.
          sections: sections.map((s) => ({
            icon: s.icon,
            title: stripHanja(s.title),
            content: stripHanja(s.content),
          })),
          yearlyMeta: {
          ...
          luckyMeta,
          monthlyFlow,
          monthlyHints: monthlyHints ? monthlyHints.map(stripHanja) : null,
          yearlyKeywords: yearlyKeywords ? yearlyKeywords.map(stripHanja) : null,
          yearlyBigEvents: yearlyBigEvents && yearlyBigEvents.length >= 2
            ? yearlyBigEvents.map((e) => ({
                ...e,
                label: stripHanja(e.label),
                description: stripHanja(e.description),
              }))
            : null,
