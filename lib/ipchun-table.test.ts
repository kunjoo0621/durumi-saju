 ...
 * 발행값 출처: 만세력 24절기표(한국시간). 2020·2024 는 정확 일치, 2026·2030 은 1분 차(반올림).
 *
 * 허용 오차 5분 — 발행처마다 반올림 관행이 달라 ±1~2분은 정상이다.
 */
// 표 전 구간(2020~2040)에 걸쳐 4개 표본을 발행 자료와 대조했다. 전부 +1h 패턴 일관.
const GOLDEN: Array<[number, number, number, number, number]> = [
  // year, month, day, hour, minute (KST 발행값)
  [2020, 2, 4, 18, 3],  // 정확 일치
  [2024, 2, 4, 17, 27], // 정확 일치
  [2026, 2, 4, 5, 2],   // 1분 차(반올림)
  [2030, 2, 4, 4, 8],   // 1분 차(반올림)
];
