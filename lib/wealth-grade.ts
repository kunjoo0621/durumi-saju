...
// 개인사주 full_json.scores.재물운 → 재물운 등급의 유일한 입력값.
// resultSchema.normalizeScoreValue 와 달리 "결측"과 "실제 0점"을 구분한다: 결측이면 null 을
// 돌려 호출부(wealth start/analyze)가 0→C 로 뭉개는 대신 결제를 막고 500 을 내게 한다.
// 값은 number 이거나 { score:number } 두 형태 모두 허용(개인사주 스키마 변형 방어).
export function extractWealthScore(fullJson: unknown): number | null {
  const scores = (fullJson as any)?.scores;
  if (!scores || typeof scores !== "object") return null;
  const raw = (scores as any)["재물운"];
