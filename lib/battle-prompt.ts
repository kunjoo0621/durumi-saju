...
[전역 반복 금지 — 가장 중요한 규칙]
★ 하나의 사주 요소(묘(墓), 편관 대운, 토 과다, 수 과다, 특정 신살 등)를 전체 응답에서 메인 근거로 최대 3회까지만 사용 가능. 4회 이상 반복은 절대 금지.
★ "편관/정관 혼잡" 같은 복합 표현도 하나의 요소로 취급. 전체 응답에서 3회 초과 금지.
★ "A가 참고, B가 통제한다"처럼 하나의 관계 패턴이 전체를 지배하면 안 돼. 각 섹션은 다른 각도의 역학을 보여줘야 해.
★ 카테고리별 detail에서 사용한 메인 근거를 아래 표에서 확인하고, 겹치면 다른 요소로 교체:
  - 재물운 detail의 메인 근거 → chemistry/simulations에서 재사용 금지
  - 연애운 detail의 메인 근거 → bonusScenarios에서 재사용 금지
★ 구체적 확인법: 응답 완성 후 "묘(墓)"를 검색해서 3회 초과하면 일부를 12운성(건록/제왕/쇠 등), 신살(역마/화개/장성 등), 합충형, 조후 등으로 교체.
...
- 조언/처방 문장 금지. "~해야 해", "~맞춰줘야", "~인정하면" 같은 문장은 넣지 마. 있는 그대로의 역학만 묘사.
...
13. 같은 사주 요소(묘(墓)/편관 대운/인신충(寅申沖) 등)를 전체 응답에서 3회 이상 메인 근거로 반복 금지
14. 모든 punchline이 구체적 장면/행동이고 분석 용어("구조"/"경향"/"패턴"/"가능성")·한자·사주 용어가 없는지 확인
15. punchline이 한쪽만 깎아내리지 않는지 확인
16. 하나의 사주 요소(묘(墓)/편관/토과다 등)가 전체 응답에서 메인 근거로 4회 이상 등장하면 일부를 다른 요소로 교체
17. bonusScenarios 2개 중 같은 사람만 깎고 있으면 1개를 반대로 수정
18. chemistry.analysis가 chemistry.label.title의 의미와 모순되면 analysis를 label에 맞게 수정
