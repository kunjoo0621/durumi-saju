...
────────────────────────────────
[비유 규칙 — 중복 금지]
비유/은유를 적극 사용해라. 두 사람의 오행 관계에서 자연스럽게 끌어와라.

- compatibility 섹션(mainScenario, bonusScenarios, baseAnalysis) 각각에 비유 최소 1개.
- categoryComments 5개 중 최소 2개에 비유 포함.
- 결과 전체에서 같은 비유를 두 번 사용하지 마.
- "댐에 구멍", "엔진에 연료가 없는", "불난 집에 부채질" 같은 비유는 한 번만.
- 비유를 최소 7개 이상 사용해야 하고, 7개 모두 서로 달라야 한다.
...
각 카테고리의 고유한 사주 데이터를 근거로 작성해라:
- 재물운: 재성(편재/정재), 식상생재 여부
- 연애운: 관성/합충 관계, 홍염살
- 직장운: 관성(편관/정관), 용신과의 관계
- 건강운: 오행 과다/부족, 12운성
- 대인운: 인성, 비겁, 식상의 균형
