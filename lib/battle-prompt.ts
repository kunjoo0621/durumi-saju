...
- label: 질문형 15~25자. "~이었다면" 형태 금지.
  해당 관계에서 벌어질 핵심 갈등/특징을 질문으로.
  좋은 예: "친구였으면 누가 먼저 연락 끊을까?", "같은 팀이면 시너지 날까?"
  나쁜 예: "친구였다면", "직장동료였다면" (밋밋, 정보 없음)
...
반드시 아래 JSON 형식으로만 응답해. 다른 텍스트 없이 JSON만.
{
  ...
  "chemistry": {
    ...
    "bonusScenarios": [
      { "type": "관계유형", "label": "질문형 15~25자", "analysis": "4~5문장" },
      { "type": "관계유형", "label": "질문형 15~25자", "analysis": "4~5문장" }
    ]
  },
  "simulations": [
