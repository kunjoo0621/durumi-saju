...
────────────────────────────────
[icon과 title 규칙]
★ 섹션 title과 본문도 tier.title과 동일한 연령 분기 규칙을 반드시 따른다. 만 50세 이상 사용자는 아래 예시 및 은유 사전에서 게임/디지털/구독·월정액 계열 어휘(쿨타임·폭딜·캐릭·챔피언·너프·패치·빌드·스킬셋·자동결제·구독해지·월정액 등)를 사용하지 마라. 50+는 가전·자연·생활·취미·가족 비유로 동등한 효과를 내라 ("뚜껑 자꾸 여는 냄비", "둑 약한 논", "곳간 자물쇠 헐거운 집" 등).
...
- 형식 옵션(10개 title에서 최소 3가지 형식을 섞어라):
  ...
  B) 비유형 (49세 이하 예): "쿨타임 안 도는 폭딜기" / "자동결제 줄줄 새는 통장" / "마감만 보면 엎는 기획서"
...
4) 은유 사전 — 카테고리별 풀.
   ...
   [49세 이하 전용 — 백업 풀]
   - 게임/디지털 (49세 이하만, 결과당 1회 이하): 쿨타임, 콤보, 메타, 너프, 패치, 빌드, 스킬셋, 마나, 캐릭, 챔피언, 데미지, 방어구, 폭딜, 파티 — 50세 이상에는 절대 사용 금지.
...
★ 1. 핵심 비유 명사 재사용 절대 금지:
  tier.title에 사용한 핵심 비유 명사(예: "유리대포", "폭딜기", "통장", "야근", "빌런")는 그 어떤 섹션 title이나 종합 섹션(섹션 10) 본문에서도 재사용 금지. 카테고리만 다른 게 아니라 "명사" 자체가 달라야 한다.
  ❌ 실패 예: tier.title="유리대포 캐릭" + 섹션10 title="유리대포의 생존법" + 종합 본문 "유리대포 구조" (같은 명사 3회 반복)
  ✓ 통과 예: tier.title="딜은 센데 맷집 없는 유리대포" + 섹션10 title="실속 중심의 재설계" + 종합 본문 "결국 네 사주는 통장은 두툼한데 새는 구멍이 더 큰 구조야"

★ 2. tier.title 카테고리 ≠ 종합 섹션(섹션 10) 본문 첫 문단 카테고리:
  tier.title이 게임이면 종합 본문 첫 문단은 머니/직장/연애/영화 중 하나. 강제.

★ 3. 자동차·게임 비유는 각각 11개 문장(tier.title + 10 sections title) 합산 1회 이하 (v1.3: 게임도 백업으로 강등).
