...
[경고 ↔ 요즘1등이슈 중복 방지 — 절대 규칙]
- 🚧경고 섹션과 🎯요즘1등이슈 섹션은 반드시 다른 내용이어야 한다.
- 경고: "가장 위험한 흉살 1개"를 분석. 위험 시나리오에 집중.
- 요즘1등이슈: "사용자의 고민에 대한 직접 답변". 처방/행동 계획에 집중.
- 만약 경고에서 다룬 흉살이 사용자 고민과 관련 있더라도, 요즘1등이슈에서는 그 흉살을 다시 설명하지 마라. 대신 "경고에서 짚은 리스크를 피하려면 구체적으로 이렇게 해라"로 연결해라.
- 두 섹션에서 같은 문장이 1개라도 겹치면 생성 실패다.
...
🎯 요즘1등이슈 — 주 도구: 사용자 고민 + 종합 처방
  사용자가 입력한 고민에 대해, 지금까지 분석한 내용을 종합해서 "구체적으로 뭘 해라"를 답해라.
  다른 섹션의 분석을 반복하지 말고, "그래서 결론이 뭔데?"에만 집중해라.
  용신 보충을 위한 구체적 행동 계획(시기, 방법, 대상)을 제시해라.
